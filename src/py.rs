...
impl CoreBPE {
    ...
    fn py_decode_bytes(&self, py: Python, tokens: &Bound<'_, PyAny>) -> Result<Py<PyBytes>, PyErr> {
        ...
        if let Ok(list) = tokens.downcast::<PyList>() {
            ...
            // whole buffer (`ptr::write_bytes(buf, 0, len)`) before handing it to
            // us, but we then overwrite every one of those bytes with the decoded
            // content, so that memset is pure redundant work — it shows up as a
            // measurable, memory-bound `memset` in the decode profile. Writing
            // into the fresh (uninitialised) buffer exactly once removes it. This
            // is sound because we write precisely `total_len` bytes, fully
            // covering the buffer, before the object is observed.
            unsafe {
                let pyptr =
