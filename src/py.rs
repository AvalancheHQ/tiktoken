...
impl CoreBPE {
    ...
    fn py_decode_bytes(&self, py: Python, tokens: &Bound<'_, PyAny>) -> Result<Py<PyBytes>, PyErr> {
        ...
        // protocol object) and fuse the token extraction with the byte lookup:
        // each token is read once and its bytes are appended straight to the
        // output buffer. This avoids materialising an intermediate `Vec<Rank>`
        // (a separate allocation plus a full second traversal of every token in
        // `decode_bytes`), which profiling showed to be a large, memory-bound
        // part of decode. The output buffer is sized at ~6 bytes/token, which
        // comfortably covers the measured average bytes/token of natural-language
        // text so the whole decode completes without a mid-loop reallocation.
        // Anything that is not a concrete `list` falls back to the original
        // generic extraction so behavior is unchanged.
        if let Ok(list) = tokens.downcast::<PyList>() {
            let mut ret: Vec<u8> = Vec::with_capacity(list.len() * 6);
